...
import { ImageData, ImageManagerProps } from "../types"

// Default upload function - replace with your own implementation
const defaultUploadFunction = async (file: File): Promise<ImageData> => {
  // This is a placeholder - implement your own upload logic
...
export function ImageManager({
  ...
  const handleFileUpload = useCallback(
    async (files: FileList) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          const imageData = await uploadFunction(file)
    ...
    [acceptedTypes, maxFileSize, uploadFunction, maxImages],
  )
  ...
  const isImageSelected = useCallback(
    (imageId: string) => {
      return selectedImages.some((img) => {
        ...
        const galleryImage = uploadedImages.find(uploaded => uploaded._id === imageId || uploaded.id === imageId)
