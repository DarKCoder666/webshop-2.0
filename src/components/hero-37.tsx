    ...
    <section className={cn("relative overflow-hidden", className)}>
      ...
      <div className="mx-auto max-w-5xl px-6 py-24 text-center md:py-32">
        ...
        <AnimatedGroup preset="slide" className="mt-6 flex items-center justify-center gap-3">
          {primaryCta?.label ? (
            <Button asChild className="h-10 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90">
              <a href={primaryCta.href}>
                <span className="mr-1 inline-flex size-5 items-center justify-center rounded-md bg-white/10">
                  <svg
          ...
          {secondaryCta?.label ? (
            <Button asChild variant="outline" className="h-10 rounded-lg px-4 text-sm font-medium">
        ...
        <div ref={containerRef}>
          <AnimatedGroup preset="zoom" className="relative mx-auto mt-20 flex max-w-5xl items-end justify-center -space-x-8 md:-space-x-12">
            {images && images.length >= 3 ? [images[0], images[1], images[2]].map((img, i) => (
              <motion.div
                key={i}
                className={cn(
