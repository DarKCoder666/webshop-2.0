...
function CarouselContent({
  ...
  const { index, setIndex, setItemsCount, disableDrag, infinite, autoplay, autoplayIntervalMs } = useCarousel();
  ...
  useEffect(() => {
    if (!autoplay || itemsLength <= 1 || isHovered) return;
    const id = setInterval(() => {
      if (!reverse) {
        const nextIndex = index + 1;
        if (infinite) {
          setIndex(nextIndex % itemsLength);
        ...
        const nextIndex = index - 1;
        if (infinite) {
          setIndex((nextIndex + itemsLength) % itemsLength);
        } else if (nextIndex >= 0) {
          setIndex(nextIndex);
        }
      }
  ...
  }, [autoplay, autoplayIntervalMs, index, itemsLength, infinite, isHovered, reverse, setIndex]);
  ...
  return (
    <motion.div
      ...
      animate={{
        translateX: `${reverse ? '' : '-'}${index * (100 / visibleItemsCount)}%`,
      }}
      ...
      className={cn(
        'flex items-center select-none touch-pan-y',
